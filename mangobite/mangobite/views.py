...
def homepage(request):
    ...
    b9=Imageclass.objects.get(name='threatsec')
    b10=Imageclass.objects.get(name='fdbsec')
    return render(request,'home.html',{'obj1':b1, 'obj2':b2, 'obj3':b3, 'bg':b4, 'bk': b5, 'sky':b8, 'threatback':b9, 'fsec':b10})
...
def analysispage(request):
    myfile = request.FILES['back_img']
    mypic = request.POST.get("back_img")
    dangers=['Fire','Reptile','Snake','Rodents','Storm','Thunder','Cloud','Flood','Rain','Crow','Animal','Bird','Wildlife']
    key_words=""
    ...
    for label in labels:
        if label.description in dangers:
            key_words+=label.description+"/"
            threats='Dangerous'
    
    threatback=Imageclass.objects.get(name='thback')
    
    if threats=='Safe':
        key_words='No Immediate Threats'

    return render(request, 'alpage.html',{'pic':uploaded_file_url, 'status':threats, 'keywords':key_words, 'tbak':threatback}) 
...
def soilanalysispage(request):
    ...
    if data['cod']=='404':
        url = 'http://api.openweathermap.org/data/2.5/forecast?q={}&APPID=406e0137603a69be44bee1e86b44881d&units=metric'.format(state)
        response=requests.get(url)
        data=response.json()

    if data['cod']=='404':
        url = 'http://api.openweathermap.org/data/2.5/forecast?q=Delhi&APPID=406e0137603a69be44bee1e86b44881d&units=metric'
        response=requests.get(url)
        data=response.json()
    ...
    def demand():
        ...
        data['dem']=(data['rate_yr_1']+1.2*data['rate_yr_2']+1.4*data['rate_yr_3']+1.6*data['rate_yr_4']+1.8*data['rate_yr_5']-7*data['avg'])/7
        ...
        rate1=[data.iloc[76]['rate_yr_1'],data.iloc[76]['rate_yr_2'],data.iloc[76]['rate_yr_3'],data.iloc[76]['rate_yr_4'],data.iloc[76]['rate_yr_5'],data.iloc[76]['dem']+data.iloc[76]['avg']]
        rate2=[data.iloc[75]['rate_yr_1'],data.iloc[75]['rate_yr_2'],data.iloc[75]['rate_yr_3'],data.iloc[75]['rate_yr_4'],data.iloc[75]['rate_yr_5'],data.iloc[75]['dem']+data.iloc[75]['avg']]
        rate3=[data.iloc[74]['rate_yr_1'],data.iloc[74]['rate_yr_2'],data.iloc[74]['rate_yr_3'],data.iloc[74]['rate_yr_4'],data.iloc[74]['rate_yr_5'],data.iloc[74]['dem']+data.iloc[74]['avg']]
        
        fig = plt.figure(figsize=(11,8))
        ax1 = fig.add_subplot(111)
        ax1.plot(year, rate1, label=data.iloc[76]['crop'], color='red')
        ax1.plot(year, rate2, label=data.iloc[75]['crop'], color='green')
        ax1.plot(year, rate3, label=data.iloc[74]['crop'], color='purple')
        
        ax1.legend()
        plt.xlabel('Year')
        plt.ylabel('Cost in Rs.')
        plt.title('Crop Rates from 2014-2018')
        plt.savefig('media/demand_graph.jpg')

        #grap=Imageclass()
        #grap.name='myplot'
        #grap.image='smooth_plot.jpg'
    ...
    mygraph=Imageclass.objects.get(name='mygraph')
    return render(request,'sapage.html',{'pH':ph, 'Humidity':hum, 'Temperature':temp, 'Irrigation':irr, 'Season':season, 'Crop':newans, 'State':state, 'District':district, 'Forecast':data['list'][8]['weather'][0]['description'], 'Information':betinfo, 'sab': b6, 'graphimage': mygraph, 'Infob':info})
